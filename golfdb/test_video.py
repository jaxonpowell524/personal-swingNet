...
if __name__ == '__main__':
    ...
    save_dir = 'predicted_frames/predicted_frames_for_' + base_name
    ...
    for i, e in enumerate(events):
        ...
        #cv2.imshow(event_names[i], img)    #next for loop from CHATGPT
        for i, e in enumerate(events):
            cap.set(cv2.CAP_PROP_POS_FRAMES, e)
            _, img = cap.read()
            cv2.putText(img, '{:.3f}'.format(confidence[i]), (20, 20), cv2.FONT_HERSHEY_DUPLEX, 0.75, (0, 0, 255))
            out_path = os.path.join(save_dir, f'event_{i}_{event_names[i]}.jpg')
            cv2.imwrite(out_path, img)
            print(f'Saved frame {e} for event {event_names[i]} at {out_path}')
        cv2.waitKey(0)
